...
LogExec("INICIA INTEGRACAO DO STATUS PEDIDOS DE VENDA COM A RAKUTEN - DATA/HORA: "+DTOC(DATE())+" AS "+TIME())

//---------------------------------+
// Inicia processo de envio Pre็os |
//---------------------------------+
Processa({|| aRet := u_AEcoStat(cNumOrc,lCancel) },"Aguarde...","Atualizando status dos pedido na Rakuten.")

LogExec("FINALIZA INTEGRACAO DO STATUS PEDIDOS DE VENDA COM A RAKUTEN - DATA/HORA: "+DTOC(DATE())+" AS "+TIME())
LogExec(Replicate("-",80))
ConOut("")
