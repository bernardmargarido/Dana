...
For _nX := 1 To Len(aPlpDest)
	...
	_cXmlDest += TagXml("telefone_destinatario",SubStr(RTrim(aPlpDest[_nX][5]) + RTrim(aPlpDest[_nX][6]),1,12),.T.)
	...
	_cXmlDest += TagXml("complemento_destinatario",PadR(RTrim(aPlpDest[_nX][9]),30))
	_cXmlDest += TagXml("numero_end_destinatario",PadR(IIF(At(",",aPlpDest[_nX][8]) > 0,RTrim(SubStr(aPlpDest[_nX][8],At(",",aPlpDest[_nX][8]) + 1)),"S/N"),5),.T.)
	_cXmlDest += TagXml("cpf_cnpj_destinatario",aPlpDest[_nX][10],.T.)
	_cXmlDest += '</destinatario>'

	_cXmlDest += '<nacional>'
	_cXmlDest += TagXml("bairro_destinatario",PadR(RTrim(aPlpDest[_nX][11]),30),.T.)
	_cXmlDest += TagXml("cidade_destinatario",PadR(RTrim(aPlpDest[_nX][12]),30),.T.)
	...
	_cXmlDest += TagXml("numero_nota_fiscal",RTrim(cValToChar(Val(aPlpDest[_nX][15]))))
