...
/*/
/***************************************************************/
User Function MDLOGM02()
Local   _lRet       := .T.

Private _lJob       := IIF(Isincallstack("U_MDLOGM01"),.T.,.F.)

Private _aPLP       := {}

Private _oProcess   := Nil

CoNout("<< MDLOGM02 >> - INICIO " + dTos( Date() ) + " - " + Time() )
...
Begin Transaction 
    If _lJob
        _lRet := MDLOGM02A()
    Else
        Processa({|| _lRet := MDLOGM02A()},"Aguarde...","Gerando lista de coleta." )
    EndIf
End Transaction

CoNout("<< MDLOGM02 >> - FIM " + dTos( Date() ) + " - " + Time() )
...
CoNout("<< MDLOGM02A >> - INICIO GERACAO LISTA DE POSTAGEM")

//----------------------------+
// Consulta notas disponiveis |
//----------------------------+
If !MDLOGM02Qry(_cAlias,@_nToReg)
    If !_lJob
        MsgInfo("Não existem dados para serem importados.","Dana - Avisos")
    EndIf
    CoNout("<< MDLOGM02A >> - NAO EXISTEM DADOS PARA SEREM CRIADOS")
    RestArea(_aArea)
    Return .F.
EndIf
...
While (_cAlias)->( !Eof() )
    ...
    If !_lJob
        IncProc("Aguarde, CRIANDO COLETA PEDIDO " + RTrim((_cAlias)->WSA_NUMSC5) )
    EndIf 

    CoNout("<< MDLOGM02A >> - CRIANDO COLETA PEDIDO " + RTrim((_cAlias)->WSA_NUMSC5) + " .")
