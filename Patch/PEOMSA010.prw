...
If ValType(_aParam) <> "U" 
    ...
    If _lIsGrid
        ...
        If _cIdPonto == "FORMLINEPRE" .And. _cValGrid == "SETVALUE"
            ...
            _nPItem     := aScan(_oObj:aHeader,{|x| RTrim(x[2]) == "DA1_ITEM"})
            _nPCodTab   := aScan(_oObj:aHeader,{|x| RTrim(x[2]) == "DA1_CODTAB"})
            ...
            If Type(_xType) <> "U" .And. _oModel:nOperation == 4
                ...
                If !u_DnFatM02(_oObj:aCols[_nLinha][_nPPrcVen], _oObj:aCols[_nLinha][_nPPerDesc], _oObj:aCols[_nLinha][_nPValDesc],_cCpo,@_nPrcX) 
                    ...
                    dbSelectArea("DA1")
                    DA1->( dbSetOrder(2) )
                    If DA1->( dbSeek(xFilial("DA1") + _oObj:aCols[_nLinha][_nPProd] + _oObj:aCols[_nLinha][_nPCodTab] + _oObj:aCols[_nLinha][_nPItem]))
