...
*/
	CASE _nOpc == 3
		...
		cNumPed	:=SUBSTR(__cChaveSCR,5,6)
	...
	//-------------------------------------------------------- INICIO PROCESSO WORKFLOW
		CASE _nOpc == 1		// Envio de email para aprovacao
			...
			While !SC7->(EOF()) .AND. SC7->C7_FILIAL == _cFilial .AND. SC7->C7_NUM == _cNum
				...
				nVuniIPI:=  SC7->C7_PRECO + (SC7->C7_PRECO * SC7->C7_IPI)/100
				...
				AAdd( (oHtml:ValByName( "t.11" )), Alltrim(TRANSFORM(nVuniIPI	 					,'@E 999,999,999,999.99')))  
